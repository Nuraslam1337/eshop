...
{
	global $basket, $count;
	if(!isset($_COOKIE['basket'])){ $basket = ['orderid' => uniqid()]; saveBasket(); }
	else { $basket = unserialize(base64_decode($_COOKIE['basket'])); $count = count($basket) - 1; }
}
...
function result2Array($data)
{
	...
	{
		$row['quantity'] = $basket[$row['id']]; $arr[] = $row;
	}
	return $arr;
}
...
function saveOrder($datetime)
{
	...
	{ 
	mysqli_stmt_bind_param($stmt, "ssiiisi", $item['title'], $item['author'], $item['pubyear'], $item['price'], $item['quantity'],
